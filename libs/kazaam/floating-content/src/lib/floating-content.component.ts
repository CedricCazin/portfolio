...
export class FloatingContentComponent implements AfterViewInit, OnDestroy {
    ...
    ngAfterViewInit(): void {
        this.rotatorThrottleMouseMove = fromEvent(this.rotator.nativeElement, 'mousemove')
            .pipe(
                throttleTime(10),
    ...
    handleMouseMove(event: any) {
        ...
        if (this.rotate) {
            const rotX = -((clientRect.height / 2 - (event.clientY - clientRect.y)) / 10) / this.rotateXShift;
            const rotY = (clientRect.width / 2 - (event.clientX - clientRect.x)) / 10 / this.rotateYShift;
    ...
    handleMouseLeave() {
        this.renderer.setStyle(this.rotator.nativeElement, 'transition', `transform 0.33s ease-in-out`);
